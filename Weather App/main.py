...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.temp_unit = tkinter.StringVar()
        ...
        for day in range(0, 5):
            ...
            self.forecast_temp_label.grid(
                row=4+day, column=3, padx=100, pady=(20, 0), sticky="nw")
    ...
    def change_degrees(self, _):
        ...
        if self.temp_unit.get() == "Degrees Celsius - °C":
            ...
            self.max_temperature -= 273.15
            self.min_temperature -= 273.15
            ...
            self.temperature_label.configure(
                text=f"{round(self.temperature)}°C")
            self.max_temperature_label.configure(
                text=f"{round(self.max_temperature)}°C")
            self.min_temperature_label.configure(
                text=f"{round(self.min_temperature)}°C")
            ...
            # Update temperature labels
            self.temperature_label.configure(
                text=f"{round(self.temperature)}°F")
            self.max_temperature_label.configure(
                text=f"{round(self.max_temperature)}°F")
            self.min_temperature_label.configure(
                text=f"{round(self.min_temperature)}°F")
    ...
    def change_speed(self, _):
        # Miles per hour
        wind_speed_list = []
        wind_speed_list.append(self.wind_speed)
